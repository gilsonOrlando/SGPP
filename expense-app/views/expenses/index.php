    ...
    <div id="history-container" class="container">
        ...
        if (isset($_GET['message'])) {
            if ($_GET['message'] === 'success') {
                ...
                echo ('Hubo un error en la operación. Inténtalo más tarde');
            }
        }
        ?>
...
<script>
    ...
    async function getData() {
        data = await fetch('http://cyclone-blower.000webhostapp.com/expense-app/expenses/getHistoryJSON')
            .then(res => res.json())
            .then(json => json);
        this.copydata = [...this.data];
        console.table(data);
    ...
    function renderData(data) {
        ...
        data.forEach(item => {
            ...
            databody.innerHTML += `<tr>
                        ...
                        <td>${item.date}</td>
                        <td>$${item.amount}</td>
                        <td><a href="http://cyclone-blower.000webhostapp.com/expense-app/expenses/delete/${item.expense_id}">Eliminar</a></td>
                    </tr>`;
        });
    }
</script>